...
1. Run the following command to update files to use the name of your provider (third-party: use your GitHub organization/username):
   ...
   - find and replace all instances of the boilerplate `scalebase` with the `NAME` of your provider.
   - find and replace all instances of the boilerplate `kzmake` with the `ORG` of your provider.
   - replace all instances of the `github.com/kzmake/pulumi-scalebase` repository with the `REPOSITORY` location
...
before:
  ...
  - make tfgen
builds:
- binary: pulumi-resource-scalebase
  dir: provider
  goarch:
  - amd64
  - arm64
  ...
  ignore: []
  ldflags:
  - -X github.com/kzmake/pulumi-scalebase/provider/pkg/version.Version={{.Tag}}
  main: ./cmd/pulumi-resource-scalebase/
  sort: asc
  use: git
release:
  disable: false
...
├── config.yaml
├── metadata.json
├── pulumi-scalebase_darwin_amd64_v1
│   └── pulumi-resource-scalebase
├── pulumi-scalebase_darwin_arm64
│   └── pulumi-resource-scalebase
├── pulumi-scalebase_linux_amd64_v1
│   └── pulumi-resource-scalebase
├── pulumi-scalebase_linux_arm64
│   └── pulumi-resource-scalebase
├── pulumi-scalebase_windows_amd64_v1
│   └── pulumi-resource-scalebase.exe
└── pulumi-scalebase_windows_arm64
    └── pulumi-resource-scalebase.exe
```
